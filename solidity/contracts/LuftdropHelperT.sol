...
interface ILuftballons {
    function claimLuft(uint256[] memory tokenIDs) external;
}
...
contract LuftwrapHelperT is IERC1155, Ownable, IERC1155Receiver, IERC721Receiver {
    using strings for *;
    ...
    constructor(address luftballons, IToken luft) {
    ...
    function onERC721Received(address, address from, uint256 tokenId, bytes calldata) public returns (bytes4) {
        ...
        uint256 balance = receipt_token.balanceOf(address(this));
        if(balance < 10000000000000000) {
            require(msg.sender == luft_ballons, "Not Enough Luft");
        } else {
            require(receipt_token.transfer(from, 10000000000000000), "Failed Transfer");
        }
    ...
    function onERC1155Received(address, address from, uint256 tokenId, uint256 quantity, bytes calldata) public returns (bytes4) {
        ...
        uint256 balance = receipt_token.balanceOf(address(this));
        require(balance >= 10000000000000000, "Not Enough Luft");
        require(receipt_token.transfer(from, 10000000000000000), "Failed Transfer");
    ...
    function claimLuft(uint256[] memory tokenIDs) external {
        ILuftballons(luft_ballons).claimLuft(tokenIDs);
    }
    ...
    function name() public pure returns (string memory) {
        return "LuftWrapped Tax Harvest";
    }
